...
function configComment(
  ...
  if (user.id === loggedUser.id) {
    configCommentBehaviour(contentContainer, "Я", "Редактировать", () =>
      redactHandler({ id, node: copy, answerTo })
    );
  } else {
    configCommentBehaviour(
      contentContainer,
      user.username,
      "Ответить",
      answerHandler
  ...
  function configCommentBehaviour(
    contentContainer,
    username,
    buttonName,
    onClickCb
...
export default function outComments(newsId) {
  ...
  comments.forEach((comment) => {
    ...
    comment.subComments?.forEach((subComment) => {
      console.log("running");
      const subCommentCopy = configComment(
        subComment,
        subCommentContainer,
        redactSubComment,
        () => answerHandler(comment.id, copy.children[1].children[5])
...
function configHideShowButton(contentContainer) {
  const showContent =
    '<img src="./public/arrow-down-black.svg" alt="">Показать все ответы';
  const hideContent =
    '<img src="./public/up-arrow.svg" alt="">Скрыть все ответы';
